...
Date.prototype.toWeiXinString = function () {
  ...
  const monday = new Date(today); // 星期一的时间
  ...
  } else if ((today.getFullYear() - this.getFullYear()) > 0) {
    str = this.format("yyyy.M.d ")
  } else {
    const hour = ["凌晨", "早上", "下午", "晚上"];
    const h = this.getHours();
    if (h == 12) str = "中午";
    else str = hour[parseInt(h / 6)];
    str = this.format("M月d日 ") + str;
  }
  str += this.format("HH:mm");
  return str;
}
console.log("======");
console.log(new Date(2020, 6, 8, 24, 0).format("yyy-MM-dd HH:mm:ss.SSS (q)(E)(x1)(x2)(x3)"));
console.log(new Date(2019, 3, 20, 24, 0).toWeiXinString());